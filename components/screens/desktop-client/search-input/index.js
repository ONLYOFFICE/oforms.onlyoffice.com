...
const SearchInput = ({ t, locale, setHideCategorySelector, theme }) => {
  ...
  return (
    <StyledSearchInput $locale={locale} $theme={theme} className={`search ${isOpen ? "open" : ""}`}>
      <button onClick={handleShowSearch} className="search-input-btn search-btn" type="button">
      ...
      <input
        ...
        value={inputValue}
      />
      <button onClick={handleClearValue} className={`search-input-btn cross-btn ${showCrossBtn ? "show" : ""}`} type="button">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">

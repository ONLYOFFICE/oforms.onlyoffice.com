    ...
    <StyledDesktopPopup onClick={() => setPopupActive(false)} className={popupActive ? "show": ""}>
      <div className="popup-container">
        <div onClick={(e) => e.stopPropagation()} className="popup-wrapper">
          <button onClick={() => setPopupActive(false)} className="popup-close-btn" type="button">
          ...
          <div className="popup-body">
            <Heading className="popup-heading" level={3} size={4} label={t("ONLYOFFICE Desktop Editors not installed")} />
            <Text className="popup-text">{parse(t("Please download it and install to use this plugin. Go to the Download page to get ONLYOFFICE Desktop Editors for Windows, Linux, or macOS."))}</Text>
            <InternalLink id="popup-btn" className="popup-btn" href={`https://www.onlyoffice.com${(locale === "en" || locale === "ar") ? "" : `/${locale}`}/download-desktop`} label={t("Get ONLYOFFICE desktop")} />
          </div>
        </div>
      </div>
    </StyledDesktopPopup>

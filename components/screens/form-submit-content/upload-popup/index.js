        ...
        <div className="popup-wrapper">
          ...
          <div>
            <div className="upload-name">
              <span className={`upload-name-file ${fileName?.match(/\.(\w+)$/)?.[1]}`}>{fileName?.match(/\.(\w+)$/) ? fileName.substring(0, fileName.length - fileName.match(/\.(\w+)$/)[0].length) : fileName}<span>{fileName?.match(/\.(\w+)$/)?.[0]}</span></span>
            </div>

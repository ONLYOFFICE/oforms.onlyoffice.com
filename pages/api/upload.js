...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    try {
      ...
      const currentLanguage = hasLanguage
        ? language === "en"
          ? ""
          : `${language}/`
        : "";
      ...
      const compressedData = zlib.deflateSync(
        `${templatePreviewRequest.data.fileUrl};${pageCount};${fileName};${fileSize};${formName};${fileRequest.data.fileUrl}`
      );
      ...
      return res
        .status(200)
        .send(
          `${CMSConfigAPI}${`${currentLanguage}`}form-submit?index=${compressedString}`
        );
    } catch (error) {

...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      const fileNameSubstring = fileName.substring(
        0,
        fileName.length - fileName?.match(/\.(\w+)$/)?.[0].length
      );
      ...
      const cardPreviewRequest = await axios.post(
      ...
      const cardDesktopPreviewRequest = await axios.post(
        `${process.env.EDITOR_API_URL}/ConvertService.ashx`,
        cardDesktopPreviewPayload,
        {
          headers: {
      ...
      const desktopPreviewRequest = await axios.post(
        `${process.env.EDITOR_API_URL}/ConvertService.ashx`,
        desktopPreviewPayload,
        {
          headers: {

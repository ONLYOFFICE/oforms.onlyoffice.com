...
export const getServerSideProps = async ({ locale, ...ctx }) => {
  ...
  const res = await fetch(
    `https://oforms.onlyoffice.com/dashboard/api/oforms/?filters[categories][urlReq][$eq]=${urlReq}&locale=${locale}&sort=name_form:${sort}&pagination[pageSize]=${pageSize}&pagination[page]=${page}&populate=file_oform&populate=card_prewiew`
  );
  const resCategory = await fetch(
    `https://oforms.onlyoffice.com/dashboard/api/categories/?filters[urlReq][$eq]=${urlReq}&locale=${locale}`
  );
